...
#Method to check the courses registered to the student
    def check_student_courses(self, student_id, course_name):
        if student_id in self.registered_students: #Checks if student id that was passed as an argument is in the registered student dictionary
            data = self.registered_students[student_id] #assigns the specific student object to the variable data
            # loops through the course list in the object assigned to data and checks if a course already exists and returns True or False
...
#Method for checking if the student ID entered exists
    ...
    def enroll_in_course(self, student_id, course_id):
        ...
        avail_courses = self.get_course_name(course_id) #Calls on the get course name method and assigns the result to available course variable
        if avail_courses is False:
            print("No course found with the course ID entered")
            return

        if self.check_student_courses(student_id, avail_courses) is True: #Calls on the check student course method and returns True if the student is already enrolled in a course
            ...
            for course in self.courses:
                if course.course_id == course_id:
                    student_data.courses.append(course.course_name) #Adds a new course to the student list
                    student_data.balance += course.fee #Updates student balance
                    print("\nCourse successfully added to student list")

#Method to calculate payments towards a course and for the required 40% payment to be made to be qualified for registration
    def calculate_payment(self, student_id, user_payment):
        ...
        if user_payment >= req_to_register:
            student_data.balance - user_payment #Reduces student balance by amount paid
...
#Method for showing students registered in a course
    def show_students_in_course(self, c_id):
        ...
        for key, value in self.registered_students.items(): #Loops through the registered_students dictionary
            if c_name in self.registered_students[key].courses: #Checks if student is enrolled in a course
                print("Student Name: ", self.registered_students[key].student_name) #prints student name


system = RegistrationSystem() #RegistrationSystem Class instantiated
...
# Conditional statements within While loop
    if choice == "1":
        ...
        except ValueError: #Error handling to prevent user from entering an incorrect value
        ...
        try:
            ...
            system.add_course(course_id, course_name, fee) #Calls on add_course method in the registration class
        except ValueError:
        ...
        try:
            ...
            while sub_menu: #Nested While loop
